...
- **键入合并**：同一厚度输入框的连续键入合并为一项；岩性下拉切换、增删层、
  载入示例等离散操作各自成项。任何新编辑都会截断重做分支；撤销后停在分支点
  继续键入会另起新项，因此再次撤销必定先停在分支点，而不会跳过它退回更早值。
- **既有规则不变**：回退/重做与普通编辑一样使结果、错误、选中步骤与比较态
  失效，并递增加请求序号使在途响应作废；重新提交后重新计算，输入相同则
  指纹与逐步证据不变。
- **双槽检查点**：历史通过浏览器内置存储保存为两个槽位的检查点，每次保存
  交替写对侧槽，附带递增代次、时间戳与 FNV-1a 校验值。代次只依据完好槽递增
  （损坏槽伪造的超大整数不会污染编号，同代次并列时以时间戳更新者为准）。写入
  中断、尾项 JSON 损坏、校验值不符或版本不识别时，选取另一槽中最近的完整代次
  恢复，并在操作区说明忽略了损坏尾项；两个槽都不可用时保留当前初始数据、禁用
  恢复并说明每个槽的原因。关闭存储权限时自动降级为当前会话内撤销/重做，刷新后
  不保留草稿。
